...
const files = ['package.json', 'package-lock.json', '.env.example', 'api/_web.js', 'api/_mcp-auth.js', 'api/mcp.js', 'api/_state.js', 'build-latest.mjs', '.github/workflows/jekyll-docker.yml'];
...
const authPatched = mcpAuth
  .replace("const resource = 'https://500900.website/api/mcp';", "const resource = new URL('/api/mcp', appBaseUrl()).toString();")
  .replace('let jwks;', "let jwks;\n\nexport function appBaseUrl() {\n  const explicit = process.env.PUBLIC_BASE_URL || process.env.APP_BASE_URL || process.env.NEXT_PUBLIC_SITE_URL;\n  if (explicit) return explicit.replace(new RegExp('/+$'), '');\n  if (process.env.VERCEL_URL) return `https://${process.env.VERCEL_URL}`;\n  return 'https://500900.website';\n}")
...
await put('api/_state.js', state
  .replace("import { get, put } from '@vercel/blob';", "import { del, get, list, put } from '@vercel/blob';")
